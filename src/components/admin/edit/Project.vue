<template>
  <div class="ad">
    ...
    <div class="ad__header">Редактировние Проекта</div>
    ...
    <form @submit.prevent class="add" v-show="!loading">
      ...
      <div class="add__img">
        <hr />
        <div class="add__header-img">Изменить обложку</div>
        <img :src="imageUrl == ''? project.img : imageUrl" height="180px" />
        <input
          type="file"
          ...
          >
            <div class="imgs__item" v-for="(slide,index) in project.imgs" :key="index">
              <template v-if="slide.img !== ''">
                <img :src="slide.url == '' ? slide.img: slide.url" class="imgs__img" />
...
export default {
  beforeMount() {
    ...
    this.$store.dispatch("portfolio/loadProjectById", id).then(() => {
      let loadProject = this.$store.getters["portfolio/project"];

      this.project.title = loadProject.title;
      this.project.subTitle = loadProject.subTitle;
      this.project.img = loadProject.img;
      this.project.imgs = loadProject.imgs;
      this.project.aboutCompany = loadProject.aboutCompany;
      this.project.task = loadProject.task;
      this.project.nameOfService = loadProject.nameOfService;
      this.projectId = id;
    });
  },
  data() {
  ...
  methods: {
    ...
    send() {
      ...
      this.$store.dispatch("portfolio/editProject", {
